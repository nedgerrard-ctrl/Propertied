...
export async function GET() {
  ...
  const posts = await VipPost.find().sort({ createdAt: -1 }).lean();
...
export async function POST(req: NextRequest) {
  ...
  const existing = await VipPost.findOne({ slug: slugBase }).lean();
  if (existing) {
    return NextResponse.json(
      { error: "A VIP post with this slug already exists." },
      { status: 400 }
    );
  }
  ...
  return NextResponse.json({
    message:
      status === "published"
        ? "VIP post published successfully."
        : "VIP post saved as draft successfully.",
    post,
  });
}
