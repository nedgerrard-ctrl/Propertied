...
export default function AdminVipPage() {
  ...
  async function deletePost(post: VipPost) {
    ...
    const res = await fetch(`/api/admin/vip/${post._id}`, { method: "DELETE" });
    const data = await res.json();
    setDeletingId(null);

    if (res.ok) {
      setMessage(data.message || "VIP post deleted successfully.");
      setPosts((prev) => prev.filter((item) => item._id !== post._id));
    } else {
      setErrorMessage(data.error || "Failed to delete VIP post.");
    }
  }

  async function togglePublish(post: VipPost) {
    ...
    const res = await fetch(`/api/admin/vip/${post._id}/${action}`, { method: "PATCH" });
    const data = await res.json();
    setTogglingId(null);

    if (res.ok) {
      setMessage(data.message || "VIP post updated successfully.");
      setPosts((prev) =>
        prev.map((item) =>
          item._id === post._id
            ? { ...item, status: post.status === "published" ? "draft" : "published" }
            : item
        )
      );
    } else {
      setErrorMessage(data.error || "Failed to update VIP post.");
    }
  }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex items-center justify-between gap-6">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold text-neutral-900">Manage VIP Contents</h1>

            <p className="mt-1 text-sm text-neutral-500">
              Create, edit, publish, unpublish, and delete VIP content articles.
            </p>
          </div>

          <Link
            href="/admin/dashboard/vip/new"
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-neutral-100">
                {posts.map((post) => (
                  <tr
                    key={post._id}
                    className={`transition ${post.status !== "published" ? "bg-neutral-50/80" : ""}`}
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 whitespace-nowrap">
                        <Link
                          href={`/admin/dashboard/vip/${post._id}`}
        ...
        <p className="mt-4 text-[11px] text-neutral-400">
          Draft posts are hidden from the client VIP page. Only Live posts appear to eligible clients.
        </p>
      </div>
    </main>
  );
