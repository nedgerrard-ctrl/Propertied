...
function validateBlog(form: BlogFormData): ValidationErrors {
...
export default function BlogForm({
  ...
  const [form, setForm] = useState<BlogFormData>({
    ...
    content: initialData?.content?.length
      ? initialData.content
      : [{ ...emptySection }],
  });
  ...
  const errors = useMemo(() => validateBlog(form), [form]);
  ...
  function addSection() {
    setForm((prev) => ({
      ...prev,
      content: [...prev.content, { ...emptySection }],
    }));
  }
  ...
  async function handleMainImageUpload(file?: File) {
    ...
    try {
      ...
      showToast("success", "Main blog image uploaded successfully.");
    } catch (error) {
      showToast(
        "error",
  ...
  async function handleSectionImageUpload(index: number, file?: File) {
    ...
    try {
      ...
      showToast("success", `Section ${index + 1} image uploaded successfully.`);
    } catch (error) {
      showToast(
        "error",
        error instanceof Error ? error.message : "Image upload failed."
  ...
  async function save(status: "draft" | "published") {
    ...
    const currentErrors = validateBlog(form);
    ...
    const payload = {
      ...
      content: form.content.filter((section) => section.body.trim()),
    };
    ...
    showToast("success", data.message || "Blog post saved successfully.");

    if (!initialData?._id) {
      router.push(`/admin/dashboard/blogs/${data.post._id}`);
    }
  }
          ...
          <div className="grid gap-5">
            ...
            <div className="grid gap-5 md:grid-cols-3">
              <label className="grid gap-2">
                ...
                />
                <ErrorText
                  message={touched.publishDate ? errors.publishDate : undefined}
                />
              </label>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-500">
                Add headings, body text, and optional images. At least one body
                section is required.
              </p>
            </div>
