...
const inputBase =
  "w-full rounded border px-3 py-2.5 text-[13px] text-neutral-800 placeholder-neutral-300 focus:outline-none transition-colors";
...
const labelClass =
  "block text-[11px] font-semibold uppercase tracking-[0.14em] text-neutral-500 mb-1.5";
...
function ProjectFormFields({
  ...
  state, setState,
  type, setType,
  propertyInterest, setPropertyInterest,
  status, setStatus,
    ...
    <div className="mt-6 space-y-4">
      <div>
        ...
        <input
          ...
          placeholder="e.g. Ascot Residences"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            className={errors.suburb ? inputError : inputNormal}
          />
      ...
      <div>
        ...
        <select
          value={type}
          onChange={(e) => setType(e.target.value as "Apartment" | "Townhouse" | "House")}
          className={inputNormal}
        >
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => setPropertyInterest(e.target.value as "off-plan" | "established")}
          className={inputNormal}
        >
      ...
      <div>
        ...
        <select
          value={status}
          onChange={(e) => setStatus(e.target.value as "Current" | "Upcoming")}
          className={inputNormal}
        >
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          ...
          <input
            ...
            className={errors.bedrooms ? inputError : inputNormal}
          />
        ...
        <div>
          ...
          <input
            ...
            className={errors.bathrooms ? inputError : inputNormal}
          />
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. 1"
            className={errors.carSpaces ? inputError : inputNormal}
          />
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          placeholder={"Walking distance to station\nRooftop terrace"}
...
function validateRequired(v: string): string {
  return v.trim() ? "" : "Required";
}

function buildProjectErrors(
  name: string, suburb: string, bedrooms: string,
  bathrooms: string, carSpaces: string, priceFrom: string, description: string
): Record<string, string> {
  return {
    name: validateRequired(name),
    suburb: validateRequired(suburb),
    bedrooms: validateNumeric(bedrooms),
    bathrooms: validateNumeric(bathrooms),
    carSpaces: validateNumeric(carSpaces),
    priceFrom: validateRequired(priceFrom),
...
function AddProjectModal({ onClose, onAdded }: { onClose: () => void; onAdded: (p: DynamicProject) => void }) {
  ...
  async function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (res.ok && data.path) { setImage(data.path); setImagePreview(data.path); }
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/projects", {
      ...
      body: JSON.stringify({ name: name.trim(), suburb: suburb.trim(), state: state.trim(), type, propertyInterest, status, bedrooms: bedrooms.trim(), bathrooms: bathrooms.trim(), carSpaces: carSpaces.trim(), priceFrom: priceFrom.trim(), description: description.trim(), highlights: highlightsArr, image }),
    });
    const data = await res.json();
    setSaving(false);
    ...
    <div className="fixed inset-0 z-[200] flex items-start justify-center overflow-y-auto bg-black/50 px-4 py-8" onClick={onClose}>
      <form onSubmit={submit} onClick={(e) => e.stopPropagation()} className="my-auto w-full max-w-xl rounded-xl bg-white p-7 shadow-2xl">
        ...
        <ProjectFormFields
          ...
          suburb={suburb} setSuburb={handleSuburb}
          state={state} setState={setState}
          type={type} setType={setType}
          propertyInterest={propertyInterest} setPropertyInterest={setPropertyInterest}
          status={status} setStatus={setStatus}
          bedrooms={bedrooms} setBedrooms={handleBedrooms}
          bathrooms={bathrooms} setBathrooms={handleBathrooms}
          carSpaces={carSpaces} setCarSpaces={handleCarSpaces}
          priceFrom={priceFrom} setPriceFrom={handlePriceFrom}
          description={description} setDescription={handleDescription}
          highlights={highlights} setHighlights={setHighlights}
          image={image} imagePreview={imagePreview}
          uploading={uploading} onImageChange={handleImageChange}
          errors={errors}
        />

        <div className="mt-7 flex justify-end gap-3">
          <button type="button" onClick={onClose} className="rounded border border-neutral-200 bg-white px-4 py-2 text-[12px] font-medium uppercase tracking-[0.14em] text-neutral-600 transition hover:border-neutral-400">
            Cancel
          </button>
          <button type="submit" disabled={saving || uploading} className="rounded bg-amber-400 px-5 py-2 text-[12px] font-bold uppercase tracking-[0.14em] text-black transition hover:bg-amber-300 disabled:opacity-50">
...
export default function BuyerInlineEditor() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/admin/content/buyer").then((r) => r.json()),
  ...
  async function confirmRevert() {
    ...
    await fetch("/api/admin/content/buyer", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(buyerDefaults),
    });
  ...
  async function save() {
    ...
    setSaving(true);
    const res = await fetch("/api/admin/content/buyer", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    setSaving(false);
    if (res.ok) {
      setContent((prev) => ({ ...prev, ...updates } as BuyerContentData));
  ...
  async function togglePublish(project: DynamicProject) {
    ...
    if (res.ok) {
      setProjects((prev) =>
        prev.map((p) => (p._id === project._id ? { ...p, published: !project.published } : p))
      );
      showToast("success", project.published ? "Project hidden from public." : "Project is now live.");
    } else {
  ...
  const investorBulletFields: Field[] = ["investorsBullet1", "investorsBullet2", "investorsBullet3", "investorsBullet4", "investorsBullet5"];
  const ownerBulletFields: Field[] = ["ownerBullet1", "ownerBullet2", "ownerBullet3", "ownerBullet4", "ownerBullet5"];
      ...
      <div className="fixed top-0 inset-x-0 z-50 flex items-center justify-between gap-4 bg-amber-400 px-6 py-3 shadow-lg">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-[12px] font-bold uppercase tracking-[0.18em] text-black">Edit Mode — Buyers</p>
          </div>
        ...
        <section className="relative bg-[#0f0c0a] py-32 overflow-hidden">
          ...
          <div className="mx-auto max-w-7xl px-6 md:px-12">
            ...
            <div className="mt-16 grid grid-cols-3 gap-8 border-t border-white/[0.08] pt-12">
              ...
              <div>
                <p className="mb-2 text-4xl font-light text-[#c8a96e]">
                  <span ref={r("stat2Value")} contentEditable suppressContentEditableWarning className={EDIT_DARK}>{c.stat2Value}</span>
                </p>
                <p className="text-[10px] uppercase tracking-[0.26em] text-[#6b5e54]">
                  <span ref={r("stat2Label")} contentEditable suppressContentEditableWarning className={EDIT_DARK}>{c.stat2Label}</span>
                </p>
              </div>
              <div>
                <p className="mb-2 text-4xl font-light text-[#c8a96e]">
                  <span ref={r("stat3Value")} contentEditable suppressContentEditableWarning className={EDIT_DARK}>{c.stat3Value}</span>
                </p>
                <p className="text-[10px] uppercase tracking-[0.26em] text-[#6b5e54]">
                  <span ref={r("stat3Label")} contentEditable suppressContentEditableWarning className={EDIT_DARK}>{c.stat3Label}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mx-auto max-w-7xl px-6 md:px-12">
            ...
            <div className="grid grid-cols-1 gap-20 md:grid-cols-2">
              ...
              <div>
                <div className="mb-8 flex items-start justify-between border-t border-[#c8a96e]/40 pt-8">
                  <h3 className="text-3xl font-light text-[#1f1a17]">
                    <span ref={r("investorsHeading")} contentEditable suppressContentEditableWarning className={EDIT_LIGHT}>{c.investorsHeading}</span>
                  </h3>
                  <span className="tabular-nums text-sm text-[#8a7b6d]">01</span>
                </div>
                <p ref={r("investorsBody")} contentEditable suppressContentEditableWarning className={`mb-8 text-[13px] font-light leading-[2] text-[#5a4a3f] ${EDIT_LIGHT}`}>{c.investorsBody}</p>
                <ul className="mb-12 space-y-4 text-[13px] text-[#2a1f1a]">
                  {investorBulletFields.map((field) => (
              ...
              <div>
                <div className="mb-8 flex items-start justify-between border-t border-[#c8a96e]/40 pt-8">
                  <h3 className="text-3xl font-light text-[#1f1a17]">
                    <span ref={r("ownerHeading")} contentEditable suppressContentEditableWarning className={EDIT_LIGHT}>{c.ownerHeading}</span>
                  </h3>
                  <span className="tabular-nums text-sm text-[#8a7b6d]">02</span>
                </div>
                <p ref={r("ownerBody")} contentEditable suppressContentEditableWarning className={`mb-8 text-[13px] font-light leading-[2] text-[#5a4a3f] ${EDIT_LIGHT}`}>{c.ownerBody}</p>
                <ul className="mb-12 space-y-4 text-[13px] text-[#2a1f1a]">
                  {ownerBulletFields.map((field) => (
                    <li key={field} className="flex gap-4">
                      <span className="shrink-0 text-[#c8a96e]">—</span>
                      <span ref={r(field)} contentEditable suppressContentEditableWarning className={EDIT_LIGHT}>{c[field]}</span>
                    </li>
          ...
          <div className="mx-auto max-w-7xl px-6 md:px-12">
            <div className="mb-10 flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-[13px] text-neutral-500">
                  {projects.filter((p) => p.published).length} published · {projects.length} total · appear on the public buyer page
                </p>
              </div>
              <button
            ...
            {projects.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded border-2 border-dashed border-neutral-700 py-24">
                ...
                <button onClick={() => setAddOpen(true)} className="mt-4 text-[12px] font-semibold text-[#c8a96e] underline underline-offset-2 transition hover:text-[#b8995e]">
                  ...
                  <div key={project._id} className={`group relative overflow-hidden border bg-[#0f0c0a] ${project.published ? "border-neutral-700" : "border-neutral-700/40 opacity-60"}`}>
                    ...
                    <div className={`relative aspect-[4/3] w-full overflow-hidden bg-neutral-900 ${!project.published ? "mt-6" : ""}`}>
                      ...
                      <div className="absolute right-3 top-3">
                        <span className={["px-2.5 py-1 text-[9px] font-medium uppercase tracking-[0.2em]", project.status === "Current" ? "bg-[#c8a96e] text-[#0a0806]" : "bg-black/60 text-[#c8a96e] border border-[#c8a96e]/40 backdrop-blur-sm"].join(" ")}>
                          {project.status}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="absolute bottom-3 right-3 flex items-center gap-2 opacity-0 transition group-hover:opacity-100">
                      ...
                      <button
                        ...
                        title={project.published ? "Hide from public" : "Show on public site"}
                      >
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/35 px-6">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
            ...
            <p className="mt-3 text-sm leading-6 text-neutral-600">
              This will reset all text content back to the original defaults and overwrite any saved changes. Projects are not affected. This cannot be undone.
            </p>
