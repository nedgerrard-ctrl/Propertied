...
export default function OffThePlanExplainerInlineEditor() {
  ...
  const [lastSaved, setLastSaved] = useState<OffThePlanExplainerContentData>(offThePlanExplainerDefaults);
  ...
  async function save() {
    ...
    if (res.ok) {
      ...
      setLastSaved((prev) => ({ ...prev, ...updates } as OffThePlanExplainerContentData));
      setSaved(true);
    } else {
