...
interface Config { lineColor: string; waveSpeedX: number; waveSpeedY: number; waveAmpX: number; waveAmpY: number; friction: number; tension: number; maxCursorMove: number; xGap: number; yGap: number; }
    ...
    function drawLines() {
      ...
      linesRef.current.forEach(points => {
        ...
        points.forEach((p, idx) => {
          ...
          const cur = moved(p, !isLast);
          ctx.lineTo(cur.x, cur.y);
          if (isLast) {
            const next = moved(points[points.length - 1], false);
